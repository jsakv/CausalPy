...
class HierarchicalDifferenceInDifferences(BaseExperiment):
    ...
    **kwargs
    ...
    grouping variable, a balanced panel, fixed random-effects group membership
    for each unit, treatment assignment at the random-effects grouping level,
    and a common adoption period for treated observations.
    ...
    def _check_treatment_group_level(self) -> None:
        treated_per_group_time = self.data.groupby(
            [self.group_variable_name, self.time_variable_name], observed=True
        )[self.treated_variable_name].nunique()
        if treated_per_group_time.max() != 1:
            raise DataException(
                "Treatment must be assigned at the random-effects group level: "
                "found within-group treated/control variation in at least one period."
            )
    ...
    def _check_clear_pre_post_periods(self) -> None:
        post_values = set(
            self.data[self.post_treatment_variable_name].dropna().unique()
        )
        if not {0, 1}.issubset({int(value) for value in post_values}):
            raise DataException(
    ...
    def _check_appropriate_data_types(self) -> None:
        ...
        for binary_column in [
            ...
            values = {
                int(value) for value in self.data[binary_column].dropna().unique()
            }
    ...
    def _fit_model(self) -> None:
        ...
        if not isinstance(self.model, PyMCModel):
            raise ValueError(
                "HierarchicalDifferenceInDifferences supports Bayesian PyMC models only."
            )
    ...
    def _compute_icc(self) -> None:
        ...
        if not hasattr(self, "model") or not hasattr(self.model, "idata"):
            self.icc = None
            return
        if (
            "sigma_random" not in self.idata.posterior
            or "sigma_fixed" not in self.idata.posterior
        ):
            self.icc = None
            return
        ...
        sigma_group = self.idata.posterior["sigma_random"].sel(
            random_coeffs=random_intercept_label
        )
        sigma_fixed = self.idata.posterior["sigma_fixed"]
    ...
    def _compute_counterfactual(self) -> None:
        ...
        if required.difference(self.idata.posterior.data_vars):
            self.y_pred_counterfactual = None
            return

        mu = self.idata.posterior["mu"]
        fixed_att = self.idata.posterior["beta_fixed"].sel(coeffs=self.did_term)
        ...
        if random_did_labels and "beta_random" in self.idata.posterior:
            obs_group_idx = xr.DataArray(
                self.group_idx,
                dims=["obs_ind"],
                coords={"obs_ind": self.X.coords["obs_ind"]},
            )
            beta_random = self.idata.posterior["beta_random"].sel(
                random_coeffs=random_did_labels
            )
    ...
    def _variance_components_posterior(self) -> xr.Dataset | None:
        ...
        components = xr.Dataset(
            {
                "random_effect_variance": self.idata.posterior["sigma_random"] ** 2,
                "residual_variance": self.idata.posterior["sigma_fixed"] ** 2,
            }
        )
    ...
    def plot_group_effects(
        ...
        **kwargs
        ...
        if not hasattr(self.model, "idata") or self.model.idata is None:
            raise RuntimeError(
                "Model must be fit before plotting random-effects group deviations."
            )
        if "beta_random" not in self.idata.posterior:
            raise ValueError(
        ...
        if random_coeff is not None:
            coords["random_coeffs"] = [random_coeff]
        ...
        icc_label = ""
        if self.icc is not None:
            icc_label = f"; ICC = {float(self.icc.mean()):.2f}"
        ax.set_title(f"Random-Effects Group Deviations ({hdi_prob:.0%} HDI{icc_label})")
        if show:
            plt.show()
        return fig, ax
    ...
    def plot_variance_components(
        ...
        **kwargs
        ...
        """
        if not hasattr(self.model, "idata") or self.model.idata is None:
            raise RuntimeError("Model must be fit before plotting variance components.")
        if not 0 < hdi_prob < 1:
            raise ValueError("hdi_prob must be in (0, 1).")
    ...
    def _posterior_summary(
        ...
        *,
        ...
        if (
            ...
            or not hasattr(self.model, "idata")
            or self.model.idata is None
        ):
            return None
        missing_vars = set(var_names).difference(self.idata.posterior.data_vars)
        if missing_vars:
            return None

        summary = az.summary(self.idata, var_names=var_names, coords=coords)
        columns = [
            column
    ...
    def _causal_impact_summary_stat(self, round_to: int | None = None) -> str:
        ...
        if not hasattr(self, "causal_impact") or self.causal_impact is None:
            return "Causal impact = unavailable; model has not been fit."
    ...
    def _plot_causal_impact_arrow(
        ...
        *,
        ...
        if treated_row.empty or counterfactual_row.empty:
            return
    ...
    def get_plot_data_bayesian(
        ...
        **kwargs
        ...
        if (
            hasattr(self.model, "idata")
            and self.model.idata is not None
            and "mu" in self.idata.posterior
        ):
            mu = self.idata.posterior["mu"]
